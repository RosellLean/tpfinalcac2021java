...
public class checkuser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            + "and clave = " + "'" + request.getParameter("contra") + "'");
            
            
                while(rs.next()){
                    out.println(rs.getString("usuario"));
                    out.println(rs.getString("clave")+ "<BR>");
                }
            
            if(rs.first()==false){
                out.println("No hay usuarios que coincidan con la busqueda.");
            }else {
                out.println("<h1>Bienvenid@ a tu cuenta!</h1>");
                out.println("<h1>El usuario ingresado es: " + request.getParameter("email") + "</h1>");
                out.println("<h1>La contraseña de tu cuenta es: " + request.getParameter("contra") + "</h1>");
            }
